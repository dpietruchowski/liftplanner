...
UserProfileRepositoryDb::UserProfileRepositoryDb(DbStorage& storage)
    : m_repo(std::make_unique<DbRepository>(
          UserProfileSerializer::table, UserProfileSerializer::user_id_key,
          QStringList { UserProfileSerializer::user_id_key, UserProfileSerializer::language_key,
                        UserProfileSerializer::timezone_key, UserProfileSerializer::sex_key,
                        UserProfileSerializer::sessions_key, UserProfileSerializer::experience_key,
                        UserProfileSerializer::goal_key, UserProfileSerializer::date_of_birth_key,
                        UserProfileSerializer::bodyweight_key,
                        UserProfileSerializer::unit_system_key, UserProfileSerializer::notes_key },
          storage, nullptr))
{
}
...
bool UserProfileRepositoryDb::createTable()
{
    ...
    table.ifNotExists()
        ...
        .column(Column(UserProfileSerializer::language_key)
                    .text()
                    .notNull()
                    .defaultValue(QStringLiteral("en")))
        .column(Column(UserProfileSerializer::timezone_key)
                    .text()
                    .notNull()
                    .defaultValue(QStringLiteral("UTC")))
        .column(Column(UserProfileSerializer::sex_key)
                    .text()
                    .notNull()
                    .defaultValue(QStringLiteral("other")))
        .column(Column(UserProfileSerializer::sessions_key).integer().notNull().defaultValue(3))
        .column(Column(UserProfileSerializer::experience_key)
                    .text()
                    .notNull()
                    .defaultValue(QStringLiteral("beginner")))
        .column(Column(UserProfileSerializer::goal_key)
                    .text()
                    .notNull()
                    .defaultValue(QStringLiteral("general_fitness")))
        .column(Column(UserProfileSerializer::date_of_birth_key).text())
        .column(Column(UserProfileSerializer::bodyweight_key).real())
        .column(Column(UserProfileSerializer::unit_system_key)
                    .text()
                    .notNull()
                    .defaultValue(QStringLiteral("metric")))
        .column(Column(UserProfileSerializer::notes_key)
                    .text()
                    .notNull()
